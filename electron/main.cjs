...
const LM_STUDIO_URL = process.env.PARAOS_LM_STUDIO_URL || 'http://127.0.0.1:1234';
let viteServer = null;
...
async function startInternalStaticServer() {
  ...
  const handleLmStudioRoutes = async (req, res, requestUrl) => {
    if (!requestUrl.pathname.startsWith('/v1/')) return false;
    let upstream;
    try {
      upstream = new URL(LM_STUDIO_URL);
    } catch {
      sendJson(res, 500, { error: 'Invalid LM Studio URL configuration.' });
      return true;
    }
    ...
    await new Promise((resolve) => {
      ...
      proxyReq.on('error', (err) => {
        const msg = err instanceof Error ? err.message : String(err);
        logLine(`[LMStudioProxy] ${req.method} ${targetPath} failed: ${msg}`);
        if (!res.writableEnded) {
          sendJson(res, 503, { error: 'LM Studio connection failed.', detail: msg });
        }
        resolve();
      });
  ...
  staticServer = http.createServer(async (req, res) => {
    try {
      ...
      if (requestUrl.pathname.startsWith('/v1/')) {
        if (await handleLmStudioRoutes(req, res, requestUrl)) return;
      }
