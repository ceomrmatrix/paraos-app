...
function emitWorkspaceSwitchedOnce(s, nextCwd) {
  const now = Date.now()
  const lastCwd = s.lastWorkspaceSwitchedCwd || ''
  const lastAt = Number(s.lastWorkspaceSwitchedAt || 0)
  if (lastCwd === nextCwd && now - lastAt < 2500) return
  s.lastWorkspaceSwitchedCwd = nextCwd
  s.lastWorkspaceSwitchedAt = now
    ...
    configureServer(server) {
      ...
      server.middlewares.use('/api/code/workspace/pick', async (req, res, next) => {
        ...
        try {
          ...
          emitWorkspaceSwitchedOnce(s, nextCwd)
